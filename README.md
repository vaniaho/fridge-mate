...
| Web 控制面板 | ✅ | HTTP + WebSocket，支持库存/食谱/留言板/天气/设置管理 |
...
| 音频模块 (`audio_hal`) | ❌ | 唤醒词检测、语音识别、TTS 播报 |
| 低功耗管理 (`power_manage.c`) | ❌ | PIR 红外唤醒、自动息屏/Light Sleep |
| 拍照预览页 | ⚠️ | GUI 占位，待接入 `vision_hal` |
| 全屏语音助手页 | ⚠️ | GUI 占位，待接入 `audio_hal` |
| 待机动态时钟 | ⚠️ | 当前显示静态时间，需从 RTC 刷新 |
| 设置页日期时间 Tab | ⚠️ | UI 设计中有，代码尚未实现 |
...
- **语音/拍照功能**：需等待 `audio_hal` 和 `vision_hal` 组件接入后才能使用。

## 团队分工

- **A**：GUI 界面 + 图像识别模块 (`components/gui_app/`、`components/vision_hal/`)
- **B**：语音音频模块 (`components/audio_hal/`)
